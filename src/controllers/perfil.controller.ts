...
import { Perfil , IPerfil} from "../models/perfiles.models";
import { Util} from "../util/utl";


export async function loadPostForm (req:Request, res:Response){
    try{
        const html = await new Perfil().renderPostForm();
        res.json({html});
    } catch (err) {
        console.error(err);
...
export async function Insert(req:Request,res:Response) :Promise<void> {
    try {
        const   pParams:IPerfil = req.body,
                instancia:Perfil = new Perfil(pParams),
...
export async function Update (req:Request, res:Response) {
    try{
        ...
        const   instancia =  new Perfil(<IPerfil>req.body),
