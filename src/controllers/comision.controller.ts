    ...
    import { Comision , IComision} from "../models/comisiones.models";
    import { Util} from "../util/utl";
    
    
    export async function loadPostForm (req:Request, res:Response){
        try{
            const html = await new Comision().renderPostForm();
            res.json({html});
        } catch (err) {
            console.error(err);
    ...
    export async function Insert(req:Request,res:Response) :Promise<void> {
        try {
            const   pParams:IComision = req.body,
                    instancia:Comision = new Comision(pParams),
    ...
    export async function Update (req:Request, res:Response) {
        try{
            ...
            const   instancia =  new Comision(<IComision>req.body),
